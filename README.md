...
`rampscan submission` is the rejection register. On 2026-08-31 FedRAMP announced the Class B/C submission form going live and listed, in its own words, the top reasons a submission gets rejected ([community#167](https://github.com/FedRAMP/community/discussions/167)) — a gated trust center, an incomplete package, missing rules or KSIs, JSON that does not follow the schema, and absent assessment content. The register is one section per reason, each carrying FedRAMP's sentence, the rules that make it binding, and rows. **129 rules are addressable at class b — 88 MUST and 41 SHOULD — and rampscan itself answers 13.** That denominator is the number nobody in this field publishes, and computing it means reading the force *per class*: 29 rules carry no top-level force at all, so a reader that took the field at face value would return the same denominator for every class.

Two things in that register are deliberately uncomfortable. The trust-center section prints **unmeasured, always** — whether an NDA gate stands in front of a provider's trust center is a property of a live URL, and this appliance makes no fetch, so FedRAMP's *first* listed reason is the one rampscan measures least and says so rather than scoring it clean. And the 116 rules it cannot answer are counted and named but are **not** called rejections: until a provider has somewhere to declare "not implemented, and here is why" — which is exactly what reason 3 asks for — the appliance cannot tell silence from a declaration it does not read, and 116 accusations is how a linter gets ignored. `rampscan submission` exits 1 on the rejections it can stand behind and prints the rest as the queue they are.
...
pnpm test               # 1,329 tests across 115 files; the ones wanting a tool or PocketBase skip by name
