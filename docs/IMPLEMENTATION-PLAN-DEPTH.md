...
**T3–T6 remain**, and T3 is now the blocker: the plane-identity decision (§5 decision 6), then `boundaryRemainder`/`externalSystem` on every record, then citing upstream on the three surviving overlaps — `SA-08`, `SA-22`, `SR-02 (01)`, all three of which agree with upstream independently — then the `pipeline-tools.json` contribution.
