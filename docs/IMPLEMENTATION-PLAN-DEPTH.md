...
**Wave 1 landed 2026-08-17 as the document family — `AC-01`, `SA-05`, `RA-05 (11)` — not as the seven-control set above.** §2.1b's reshaping is why: the wave authors what the preceding triage batch made available, and batch 1 left sixteen `partial` records with no recipe of which those three are one recipe shape written three times. Two deviations from this section's expectation, both recorded in the changelog: a **new collector** (`documents`) rather than recipes over existing ones, because policy and system documentation have no filename convention to read and both records say the path is *declared*; and none of the Tier-2 cheap wins, which unlock `SA-22` and `SI-12` rather than these three and are therefore wave 2's. The seven-control set above has since resolved into four states, which is why it could not be the wave on its own: `SA-02`, `SR-02 (01)` and `SR-11 (01)` are `narrative` and owe no recipe at all; `SR-08` is **retired** (upstream answered it at overlay 0.7.2 and this overlay conceded the reading); `SA-05` landed here; and only `SA-08` (over `contract`) and `SA-22` (which wants `dockle` plus a lifecycle feed the collectors cannot reach) still owe one.
...
### N1a-T3 batch 1 — the class-b controls a repository might plausibly answer (2026-08-17)

**The batch is 19, and the number was produced by a rule rather than chosen.** §2.1b says batches of ~15 class-b controls, and taking the largest-leverage fifteen would have been the obvious reading. Recomputed at overlay 0.7.2 the class-b remainder is 37, and it splits almost evenly into two sets that behave nothing alike:

```
repo-plausible families (CM SA SR SI RA CA AU SC AC IA)   19   AC 2 · CA 5 · IA 3 · RA 2 · SA 2 · SC 2 · SI 1 · SR 2
everything else                                           18   PS 7 · AT 4 · CP 3 · PL 2 · IR 1 · MA 1
```

Leverage does not separate them — 26 of the 37 sit at leverage 7 — so a batch cut by leverage lands mostly in PS, AT and CP and produces **fifteen refusals and no recipes**. That satisfies the alternation rule in letter and defeats its purpose, which is that authoring keeps pace with triage: §2.1a adopted the rule because *"triage is cheap and moves `unreviewed`, authoring is expensive and moves `covered`, and a loop that prices only the cheap one drifts."* A batch that makes no recipe available is a triage batch with a recipe batch that has nothing to be about. So batch 1 is the repo-plausible set entire, and 19 rather than 15 because the alternative cut is taste.

**The confirmation arrived on the other side.** After batch 1 the class-b remainder is exactly `PS 7 · AT 4 · CP 3 · PL 2 · IR 1 · MA 1` — batch 2 is the wall of honest refusals, and it is now identifiable as such in advance rather than discovered halfway through. That is where the auditor gate earns most: fifteen narratives written quickly is risk 3 in its purest form.

**What the command reads after the auditor pass, every figure its own:**

```
frontier 119 uncovered controls · dataset 2026.07.14.01
  adjudicated  0 automatable · 18 partial · 7 narrative
  unreviewed   94                                        (was 113)
  discharged   0        (automatable AND a recipe exists today)
  catalog covers   22 of 209 controls a KSI reaches
  reachable        37 of 209 — 17.7%                     (was 24 — 11.5%)
```

**Zero automatable, for the twenty-fifth record running.** Decision 7 predicted `automatable` would be nearly unreachable once the boundary limb was required, and nineteen more records have not found it. That is the ceiling arriving early and it should be read as the plan working rather than as the plane underperforming.

**Four records diverge from upstream, and each had to argue rather than assert.** There were five when the batch was written; the fifth is §"what the auditor took off the board" below. Ground rule 10 is cheap while the overlap is three controls; batch 1 made every one of its nineteen an overlap, because the AWS pass had already walked all ten of these families. The divergences:

- **`AC-01`** and **`RA-05 (11)`** are the plan's thesis landing on its own terms. Upstream refuses both because *"no API reports that a document exists or that anyone read it"* — and a commit **is** that report, carrying the content hash and the anchoring an API cannot produce. On `RA-05 (11)` the divergence is narrower than a disagreement: upstream's own rationale names *"the published policy, the disclosure page or security.txt"*, two of which are files under version control and the third of which is this record's remainder. The two passes agree about the control almost word for word and disagree only about whether the artifacts are reachable. Both records claimed *currency* as their sharpest addition when they were written and neither claims it now — see the auditor findings.
- **`IA-06`** reads the artifact upstream's one-sentence refusal is about. *"No API reports what a form displayed while someone typed into it"* is true of an estate and false of the source that renders the form: the masking attribute, the error-message branch and the credential-carrying log line are committed text.
- **`SR-10`** answers a limb their refusal never reached. Upstream refuses on hardware inspection under the shared model; the components a cloud system acquires are overwhelmingly software, and the value they are inspected against is a set of digests carried in a reviewed file.
- **`SA-03`** was the fifth and is **withdrawn**: it is `narrative` now, agreeing with upstream, because the auditor found the artifact its argument rested on is not in the checkout.

#### The auditor gate, first run — and it caught the record it was flagged to catch

**§2.1b's gate is a process, not a test, and this is the first batch closed by one.** Upstream's definition-of-done requires a separate agent to attack the batch and its findings reported verbatim, clean results included, on the stated ground that *"you cannot do this to your own work, because the reading that produced the claim is the reading that would check it."* The pass was given the ground rules, the schema, upstream's file and the collector sources, and told to attack over-claim, vacuity, under-claim, citation correctness, named-collector capability, self-dealing and copy-paste. It ran 32 tool calls over eight minutes and returned **7 SOUND, 11 WEAKEN, 1 RE-RATE** with ten findings ranked by severity. Every load-bearing claim in it was verified against the source before anything was changed, and every one held.

**What it certified clean, which is reported because a gate that only reports failures is a gate nobody can calibrate.** All 19 records: `source`, `datasetVersion`, `externalSystem` present, `remainder` with both limbs on every partial and absent on every narrative, and — the one most at risk from writing nineteen records in one sitting — **no copy-paste filler; every `boundary` limb and every `externalSystem` is written per control**. All 19 citations recounted against upstream's live file: every disposition matches, every `agrees`/`diverges` is legitimate, all five divergence notes argue rather than restate. It also independently recounted upstream's `pipeline` plane at 13 controls and confirmed none is in this batch, so `aws` is the correct citation target on all nineteen.

**What it took off the board.** `SA-03` was the batch's most contestable record and was flagged to the auditor as such — the divergence rested on branch protection and required status checks being *"the lifecycle's phase gates in enforced form."* The finding is that the artifact is not in the checkout: branch protection is platform API state, no collector reads repository settings, and `repo-facts`' `cacheScope` is a fixed set of filename families. Strip the unreadable half and the residue is CODEOWNERS (which the record's own remainder had already conceded is not a role assignment), a workflow file (verbatim the *"pipeline execution record"* upstream refuses), and the architecture contract (which governs imports and route auth, not what may merge, and is already claimed under CM-02/CM-06). **So `SA-03` is now `narrative` and cites upstream as agreement.** The rewritten record says the contest was attempted and why it failed, which leaves the overlay better off than never having tried: it can now tell a reader that the obvious commit-plane counter-argument to upstream's sharpest refusal has been tested and does not hold, and it adds one thing upstream's rationale does not state — the gate is not merely unreported by an estate API, it was never a file.

**The nine other findings, every one a real defect, grouped by what they were.** Four were claims about machinery that does not exist: the currency limb on `AC-01` and `RA-05 (11)` (**nothing in the collector set reads git history metadata at all**, and reviewer identity is platform state that a squash merge destroys); `SR-10`'s digest verification and provenance *verification* (`repo-facts` records `lockfile_present` and reads no entries; the workflow patterns match provenance being generated and signed, not verified before consumption); and `CA-03`/`SA-09` naming the IaC scanner for a resource inventory it does not produce — it emits failed checks and a passed count — and the spec linter for characterising an exchange it only lints. Two were the same self-dealing defect in two records: `CA-02` and `CA-07` both said the certification class is committed configuration, and it is `values.class ?? "b"`, a per-invocation flag — *the precise defect `CA-02`'s own citation levels at a conformance pack*, which is as neat a demonstration as this project is likely to get that self-review does not catch the thing you just wrote. One was an inversion: `AC-14` claimed `arch-route-auth-declared` fails an undeclared unauthenticated route, when `RouteAuthRule` scopes routes that **must** reach an auth check, so an undeclared route matches nothing, emits nothing and fails nothing — silence reading as compliance, ground rule 7's exact shape, inside a record about enumeration completeness. One was over-precision: `CA-07 (04)` claimed attribution to *"the commit and author"*, where `ports.ts` documents `introducingCommit` as the first commit rampscan **saw** the violation at and no author field exists anywhere. Two were vacuity: `IA-06` borrowed a find-the-bad-pattern construction with `empty_means: clean`, which passes on any repository with source and no login screen, and `SI-12`'s IaC baseline passes on infrastructure declaring no retention-bearing resource — plus its CloudFormation clause, which this repository's scanner does not match. The last was an under-claim gap the auditor found by reading the batch against itself: `SC-39` refused three near-miss artifacts and missed the fourth, a committed `runtimeClassName`/seccomp/`securityContext` declaration — the same *class* of artifact `SI-12` rates partial two records away.

**All ten are fixed in this commit and none of the fixes is a softening of language.** Where a claim named machinery that does not exist, the record now says so and names what would have to be built; where the vacuity was real, it moved into `remainder.control` and the recipe shape it needs is stated (`IA-06` now requires the witness-row construction `codeowners-defined` uses); where a collector could not do the job, it is out of `candidateCollectors` rather than left in with a hedge. The frontier's own numbers moved with them — `reachable` 38 → **37**, which is the gate doing the one thing a coverage push most needs and least wants.

**The gate's cost and what it bought, stated so the next batch can be priced.** One agent, eight minutes, 163k tokens, against nineteen records that took a working session to write. It found one wrong disposition, one inverted claim about our own recipe, two identical self-dealing errors, four claims about unbuilt machinery, and two vacuity holes — **eighteen of nineteen records carried at least a correction**, and the two the batch was proudest of (`AC-01`, `RA-05 (11)`) both had their headline sentence cut. Ground rule 8 has had no enforcement since it was written; it does now, and the honest reading of this first run is that self-review would have shipped every one of these.

**The refusals were taken where the evidence is strongest, which is the only place a refusal proves anything.** `RA-03 (01)` is the one to read, and the auditor rated it the best-disciplined record in the batch: a supply chain risk assessment sits next to this overlay's best artifact, and the record declines the SBOM as evidence for it in the same words upstream declines AWS Artifact — *inputs to the assessment are not the assessment*. Two passes each holding good inputs and each refusing to call them the analysis is a stronger statement about the control than either refusal alone. `CA-02 (01)` refuses assessor independence with CODEOWNERS sitting one directory away, and `SC-39` refuses process isolation with `container-runs-nonroot` in the catalog — both named in their rationales as considered and refused, so a later reader can tell refusal from oversight. `SA-03` joined them after audit, which makes four.

**A test changed shape rather than being patched, and the reason is the interesting part.** `frontier.test.ts` pinned the overlap set to `["SA-08", "SA-22", "SR-02 (01)"]` and asserted all three agreed. That was a true fact about a world where three of seven records happened to meet upstream, and it was wrong the moment a batch was written into families the AWS pass had already walked — it would have failed on any batch, which makes it a snapshot wearing an assertion's clothes. It now asserts the property (every overlap cites; every note is long enough to be an argument), and a **second** test pins the divergences **by name**. That is the half of ground rule 10 no link check can reach: `agrees` is recounted against upstream's file mechanically, but `diverges` is a claim, and a claim is only as good as someone having noticed it was made. A record filed as a disagreement without anyone deciding to now fails CI — and the list is four rather than five, shorter by exactly the record the auditor took off the board, which is the pin doing its job on its first day.

**660 vitest** green (+1: the divergence pin), root and console typecheck clean.

### N1b wave 1 — the document family, and the first recipes batch 1 made available (2026-08-17)

**The alternation rule chose this batch, and batch 1's own output chose its contents.** §2.1a adopted upstream's rule — never two triage batches in a row while an authoring backlog stands above ~15 — and after batch 1 the backlog was exactly that: **18 `partial` records across the overlay** — seven written before the sibling reconciliation, nineteen in batch 1 — of which only `SR-05` and `AC-14` named a recipe the catalog already held. Reading the sixteen unlinked ones for what they have in common rather than for leverage produces one obvious first move: **`AC-01` and `RA-05 (11)` from batch 1, joined by `SA-05` from the first set, are the same recipe three times** — a document exists at a stated path, is not empty, and is anchored so that an edit kills the claim. Three controls, one shape, over collectors that spawn nothing.

**The declaration, and why it is not a filename convention.** `SECURITY.md` and `.well-known/security.txt` are conventions someone else established and a reporter finds without being told, so `security-disclosure-published` is a filename list in `repo-facts` beside `codeowners-defined`, which is the same check about the same kind of file. Policy and system documentation have no such convention, and the two records say so in their own words — "present at a **declared** path", "the documents exist at **named** paths". So the other two recipes ride a new `documents` block in the scanned repo's `rampscan.config.json`, the L1 contract's shape and for L1's reason: a document-existence check with no declaration behind it either invents a convention nobody agreed to, or passes on any repository containing a markdown file.

**`kind` is a closed enum with exactly two values, and that is the audit's lesson applied before the fact.** A generic `"policy"` would let a repository declare its incident-response policy and collect AC-01 evidence for it — the over-claim the batch-1 audit caught twice, arriving through a config field instead of a rationale. `"access-control-policy"` and `"system-documentation"` are what wave 1 can honestly answer; a third subject is a schema change, a new recipe and a new adjudication link rather than a string somebody types.

**Two empty-set patterns, deliberately different, and the barren fixture shows the difference.** The disclosure channel is a **Witness**: one row always, so a repository publishing nothing VIOLATES — an absent SECURITY.md is a real answer, not a missing observation. The declared documents are **Guard and Negative witness**: no block at all is a stated SKIP (a repository that declared nothing has not failed a documentation control, it has made no claim), a block declaring only one kind omits the other kind's observation key entirely, and a document declared but *missing* emits `present: false` and violates — contract.ts's posture for a rule that matches nothing, because a declaration pointing at a path that is not there must fail the thing it mistyped. `bare-app`'s pinned split moves **5/4/8 → 5/5/10** and the new pin says which recipe went where and why, since the whole value of a pinned baseline is that movement arrives with a paragraph.

**Currency is still not claimed, and one record had to be corrected to keep it that way.** `SA-05`'s record predates the batch-1 audit and still said rampscan can evidence "when they last changed, which commit changed them and who reviewed that change". That is the exact claim the audit cut from `AC-01` and `RA-05 (11)` — **nothing in the collector set reads git history metadata**, and reviewer identity is platform state a squash merge destroys — and it survived only because SA-05 was written in the first seven and the audit ran over the nineteen. Writing its recipe is what read it again. The rationale now names the absence and the currency limb moved into `remainder.control`, which is the second time the same finding has had to be made and the argument for the auditor gate covering authoring batches too.

**Writing the first `recipeIds` found a link nobody had ever walked backwards.** `api-spec-lint-clean` has claimed `sa-5` since the H phase; SA-05's record named no recipe at all. The frontier derives the two sides independently — `catalogRecipeIds` from the catalog, `commit.recipeIds` from the record — and its own comment has always said that *a catalog recipe the record forgot to claim is a broken link too*, but nothing checked that direction, because until this batch every record's `recipeIds` was empty and there was no direction to disagree in. `frontierProblems` now walks it, with a synthetic pin in `frontier.test.ts`; SA-05 names both recipes; and the check matters more on a future `narrative` than it did here, where it would catch a flat contradiction — the record saying a repository cannot answer a control the catalog claims.

**What the command reads, and the number that did NOT move.**

```
frontier 119 uncovered controls · dataset 2026.07.14.01
  adjudicated  0 automatable · 18 partial · 7 narrative
  discharged   0        (automatable AND a recipe exists today)
  catalog covers   24 of 209 controls a KSI reaches     (was 22)
  reachable        37 of 209 — 17.7%                    (unchanged)
```

Three recipes moved `catalogCovered` by **two**, because `sa-5` was already inside it via the spec linter — stated here because a reader recounting three against a delta of two would otherwise be right to suspect a miscount. `reachable` did not move at all and `discharged` is still zero, and both are the plan working rather than the wave underperforming: all three controls were already `partial`, so the adjudication had already counted them as answerable, and **N0 decision 3 says a partial never discharges**. N1b's exit — "`frontier` moves by exactly the number of controls the new recipes discharge" — is therefore satisfied at zero, which is the honest reading of a wave that shipped real evidence for three controls it still cannot close.

**`rampscan check` picked the collector up on its own, and that is the wave's quietest win.** `documents` spawns no process and consumes no artifact, so `dryRunnable` admits it by the rule that already admits `repo-facts`, `graph` and `contract` — meaning a declared document deleted in an uncommitted change fails the pre-commit dry run *before* the commit exists. The check test's expectation moved from three names to four with the reason written down rather than the list quietly re-pinned.

**Dogfooded, including the part that reads badly.** `rampscan.config.json` now declares three system-documentation entries (`docs/ARCHITECTURE.md`, `docs/SPEC.md`, `README.md`), so the self-scan evidences `system-documentation-present`; `access-control-policy-present` reads **unevidenced** on our own board because this repository has no access control policy to declare and the Guard says so rather than inventing one; and `security-disclosure-published` reads **violated**, because rampscan publishes no SECURITY.md. That last row is left standing on purpose — a disclosure channel is a promise that someone reads what arrives, and committing a file to turn a row green would be precisely the inert compliance the record's own remainder names.

**679 vitest** green (+19: 18 for the documents gate and the disclosure witness, 1 for the reverse-link pin), root and console typecheck clean. The fixture scan reaches `evidenced` on all three new recipes; the barren fixture reaches none of them.

**Not in this wave, and named so it is not mistaken for done:** the **history collector** (the currency limb three records now name as an absence — the single most-cited missing machinery in the overlay), `dockle`, the license recipe and `trivy config` (the Tier-2 wins that unlock `SA-22` and `SI-12` rather than these three), and the **N1b fork** — whether N2a jumps ahead of N1c — which §2's rule prices on collector churn: wave 1 took one new collector plus one extension, so the recommendation is to hold the gate until wave 2 rather than ship it against a catalog still moving.

### N1b′ — the overlap nobody could see, and the pin that moved twice in one day (2026-08-18)

**The re-pin came first and it is the finding that reframes the rest.** N1a′-T1 re-pinned `automation-frontier.json` 0.6.0 → 0.7.2 on 2026-08-17 and the plan recorded that as risk 8 closed. Upstream is at **0.7.5** and the frontier reads **112**, not 119 — seven controls left the uncovered set while wave 1 was being written, because upstream authored seven more recipes on its own plane. So the lesson of T1 was not "the second pin was missing", which is fixed and worked exactly as designed. It is that **upstream ships overlay bumps faster than this project closes a batch**, and a re-pin therefore belongs at the START of a batch rather than at the end of one. Re-pinning at the end is re-pinning after the batch has already reasoned against a stale file. `pins.ts` says so now in the place a reader will be standing when it matters.

**Two hardcoded counts came out of `frontier.test.ts` on the way, and the reason is batch 1's own.** The suite pinned `frontierTotal` at `119` and the rendered header at `frontier 119 uncovered controls`. That number has been 121, 119 and 112 inside two days and every move was upstream's to make; the loader already refuses to load a moved overlay at all, so the literals caught nothing the pin had not already caught and cost an edit in a second file every re-pin. This is precisely what batch 1 said about the overlap pin — *a snapshot wearing an assertion's clothes* — and the fix is the same one: assert the property the literal stood in for. The map is now checked as a **lossless fold** of the pinned file, and the header is checked to quote **the number it computed**, which is ground rule 9 at the point of display.

#### The hole: ground rule 10 was pointed at the one file where the fact cannot appear

`citesUpstream` is checked against `automation-frontier.json`. **The frontier is the UNCOVERED set.** So the check fires wherever upstream *adjudicated* a control and is structurally blind wherever upstream *answered* one — answering it removes the row, and the row is the only place the check looks. Three controls had been sitting in that blind spot:
...
sa-11    ours no-reachable-dangerous-code   theirs static-analysis-coverage-and-flaw-disposition
         ours tests-in-ci                   theirs static-analysis-coverage-and-flaw-disposition
si-10    ours no-reachable-dangerous-code   theirs input-validation-taint-analysis-coverage
```

Neither project said a word about the other on any of the three, and **no check on either side could have said one.** N1a′-T5 turned ground rule 10 from a norm into a link check and the link check was complete for the file it read. This is not that check failing; it is that check pointed at the wrong file, and the right file is upstream's **evidence plan** — their published recipes — rather than their frontier, which is their published gaps.

**So the client reads a third file and takes a third pin.** `evidence-plan.json` carries `provesControls` per recipe with the plane that authored it. Two things had to be got right and both were nearly wrong: `provesControls` is scoped **per certification class**, so one recipe appears under several classes with the list populated in some and empty in others and reading a single class under-reports — on a check about undeclared overlap, under-reporting *is* the failure, so the map unions across classes and a test pins that it does. And the slice carries **no root `overlay_version`** to pin, because upstream deliberately removed it: a single page-level stamp over a mixed-plane population is how a class page prints one version over two overlays. What it carries instead is one stamp per plane, one level down, so `DEFAULT_PLANE_PINS` is keyed by plane rather than by file — the same reason `DEFAULT_OVERLAY_PINS` is keyed by slice, arriving one level further down. `PlaneVersionMismatchError` is a third error class rather than a reuse, because the reading it calls for is a third one: a plane bump means upstream authored or withdrew recipes, so **the right response is to re-read the CATALOG, not the records**, and a reader who cannot tell that from the message will do the other job.

**The pin refuses at the moment a new overlap becomes possible.** That is the whole design: not after one has shipped undeclared, but the first time the file that could create one moves.

#### The check is scoped to upstream's `pipeline` plane, and the scoping is the decision

Run unscoped, the new arm reports **twenty-five** overlaps with `aws` on the first pass — `ra-5`, `si-2`, `cm-2`, `ac-6`, `sc-28` and the rest — and every one is the product's own thesis rather than a finding. An AWS recipe and a commit recipe over one control are different evidence *by construction*: an API over a running estate against a file in a checkout, and no reader could mistake one for the other. Twenty-five declarations all saying that is the thesis restated twenty-five times, and a gate red on its first run for reasons nobody acts on is the gate N2a was deliberately held back to avoid becoming. Upstream's `pipeline` plane is the only case where the two claims are **plausibly the same**, because it is the only upstream plane whose subject is ours. `COLLIDING_UPSTREAM_PLANES` is one line, and it is where a third colliding plane would be added.

**All four overlaps are declared `corroborates`, and none of the notes is a formality.** On `ia-5.6` upstream reads whether secret scanning and push protection are switched on and which repositories those settings reach; this plane walks every commit in one checkout's history offline. Their scope predicate is the thing our single checkout cannot supply and our history walk is the thing their alert store cannot, because an alert store holds only what a scanner that was on at the time happened to see. On `sa-11` the split is by limb — theirs is the evaluation limb, `tests-in-ci` is the testing limb, and the note says so rather than letting one control id on two catalogs read as a duplicated claim. `no-reachable-dangerous-code`'s note carries the honest counterweight in the body rather than a footnote: **our ruleset is three rules today (N3a)**, so their query-suite and language coverage is a breadth claim ours does not make, and a reader should read ours as deep and theirs as wide.

#### `sa-15.3` was an over-claim, and the sibling found it before we did

`ci-provenance-present` had claimed **SA-15 (03) — criticality analysis** since the H phase, on two assertions: that a workflow file exists, and that some step generates build provenance. Upstream's pipeline pass adjudicated the same control `narrative`, and their rationale refutes the route directly — *"the analysis names which components are critical to mission function, which is a judgement about the mission rather than a property of the build. A dependency graph enumerates components and ranks none of them."* That is correct. Provenance answers where an artifact came from; criticality answers which components matter most if they fail. And the first assertion was worse than off-target: `workflow_count >= 1` reports evidenced from **the presence of a CI directory**, which is ground rule 7's failure — a claim discharged by a file existing rather than by anything in it — sitting in the catalog since the H phase.

Both assertions are gone, `sa-15.3` is out of the recipe's `control_ids`, and the record the removal leaves owed is written and cites upstream as agreement. **The numbers moved the way a gate is supposed to move them: `catalogCovered` 24 → 23, `reachable` 37 → 36, 17.7% → 17.2%.** Coverage went down because a claim was wrong, which is the second time in three batches that an outside reading has cost this overlay a number it had already published.

The asymmetry is worth recording. The two projects reached `sa-15.3` from opposite directions — **they adjudicated it and declined to author; we authored and never adjudicated** — so nothing on either side compared the recipe against the control text until both files were read together. The divergence pin covers records against records. A recipe claiming a control upstream has refused was in nobody's field of view, on either side.

#### A refusal that points somewhere: `remainderAnsweredBy`

The most useful thing this overlay can do with a `narrative` is name who **can** answer the limb it refuses, and the empirical basis for doing so arrived this batch. Every one of upstream's nine pipeline recipes collects through `gh api` over organization and repository state — rulesets, code-security configurations, alert stores, approval records. **Not one reads a file in a checkout.** So the boundary between the two planes was never agreed; it fell out of the evidence. They read platform state, this plane reads committed bytes — and the limbs this overlay refuses *for being platform API state rather than a file* are disproportionately the limbs their recipes answer.

Two records take the pointer and both were withdrawals or trims made by an audit:

- **`sa-3`** refuses because branch protection and required status checks are not in the checkout. Upstream's `sa-10` reads the branch rules and `cm-4.2` reads whether the status-check runs were a **condition of the merge** rather than decoration beside it. That is the evidence batch 1's withdrawn contest needed and could not get, and naming it turns the withdrawal into a division of labour rather than a dead end.
- **`sr-10`** names verification-before-consumption as unbuilt here — the patterns matched today detect provenance being *generated*, not *verified*. Upstream's `si-7.7` is that verification, scoped to the artifacts a deployment record says are actually running, which this plane structurally cannot reach.

`ca-3` and `sa-9` were considered and **not** given a pointer, stated so it reads as a decision: the batch-1 audit's finding there was about our own collectors being misnamed as candidates, and upstream's external-system paragraphs are a shared caveat rather than a recipe that closes a limb.

Structured rather than prose, and unscoped to plane — the opposite way round from the overlap check, deliberately. There the `aws` case is noise; here it carries real information, because a remainder answered by an API over a running estate is a remainder the commit plane will **never** close, which is exactly what N1d's ceiling needs to say about it.

**691 vitest** green (+12), root and console typecheck clean, `rampscan frontier` exits 0.

**One defect found and not fixed, named rather than absorbed:** `packages/collectors/test/documents.test.ts` (wave 1) has individual tests running 3.2–4.0s against vitest's 5s default, so the file passes alone and times out intermittently under full-suite parallel load — it failed 8 tests on one run of this session's suite and passed on the next. It is a real flake in code about to be committed and the cause is not yet known: the fixture is 68 files and 460 KB, so the copy is not it. Raising the timeout would hide it rather than answer it, and the collector's own runtime is a product question as well as a test one.

#### The flake, root-caused before the commit — and the leak underneath it (2026-08-18)

**It was the copy, and the collector was never the suspect it was named as.** The paragraph above left the cause open and asked whether the runtime was a product question as well as a test one. Measured on the fixture, ten iterations each: `documents` is **7.5ms** median and `repo-facts` **10.3ms**. It was a test question. Everything else in those tests was `fixtureCopy`, and `fs.cp` over `vulnerable-app` measures **346ms median with a 2.7s tail** on an idle machine — the fixture's generated `.git` is 51 of its 68 files and 372K of its 460K, and Node's recursive copy walks it entry by entry. Filtered, the same copy is **137ms median / 173ms max**; under eight-way concurrent load the pair is **573ms/951ms against 101ms/226ms**. Thirteen copies in one file, any of which can hit the tail, against vitest's 5s default, is how eight tests failed on one run and passed on the next.

**Excluding `.git` is the more honest copy, not merely the cheaper one.** No collector on this plane reads git history metadata — the batch-1 audit established that by cutting the currency limb from `AC-01` and `RA-05 (11)`, and wave 1 cut it again from `SA-05` — so a copy carrying history carries something no assertion here can see, while every test using it breaks the **working tree**: a document deleted, a document emptied, a declaration rewritten. `packages/collectors/test/checkout.ts` is shared rather than local because `contract.test.ts` had the same latent cost at two more sites and would have been the next file to flake.

**And under it, a leak nobody had looked for.** Twenty-nine test files call `mkdtemp` at about eighty sites and **not one removes what it made**: a single day of running this suite had left **3,035 `rampscan-*` directories and 615 MB** in the system temp directory, so every `mkdtemp` after them was creating an entry in a directory holding thousands. That is the environmental half of a heavy tail, and cleaning up at eighty call sites is eighty chances to forget. `os.tmpdir()` reads `TMPDIR` on every call, so the fix is one root per run in `vitest-global-setup.mjs` — the same file that already holds the fixture-race fix, for the same reason it holds that one — created before the workers fork, removed when the run ends. Verified rather than assumed: a full run now adds **zero** directories to the system temp and leaves its own root behind.

**What it reads as now.** Under full-suite parallel load the slowest `documents` test is **156ms and 226ms** across two runs, against the 3.2–4.0s that flaked — roughly twenty times the headroom, bought by removing work rather than by raising the limit, which is what the finding asked for. **695 vitest** green, root and console typecheck clean. The four above N1b′'s 691 are the tool-gated tests that run when `gitleaks`, `syft` and `osv-scanner` are on `PATH`, not new tests.

**Stated because it is the limit of the evidence:** the intermittent failure itself was never reproduced in this session. What is measured is the cost that was there and is now gone, and the 3,035 directories already on disk are stale detritus this change stops producing rather than removes.

### N1a-T3 batch 2 — the class-b remainder, and the wall that turned out not to be one (2026-08-18)

**Batch 1 named this batch before it was written and got its contents exactly right and its character wrong.** Its commit message says the class-b remainder is "PS 7 · AT 4 · CP 3 · PL 2 · IR 1 · MA 1, so batch 2 is identifiable as the wall of honest refusals BEFORE it is written rather than halfway through." Recomputed from the live frontier at the head of this batch, the remainder is those eighteen controls to the family. It is not a wall. **Sixteen are `narrative` and two are `partial`**, and the two separate on a distinction that is worth more than either record: **the split falls on a verb, not on a family.**

> A control whose subject is a DOCUMENT the system is asked to develop is answerable in part from a checkout, because a repository is a document store with integrity. A control whose subject is an ACT performed on or by a person — training delivered, screening completed, an agreement signed, a sanction applied — is not, because the document a repository could hold is evidence *about* the act, and the act itself leaves nothing in a checkout.

`CP-02` asks that a contingency plan be developed and `PL-08` that a security and privacy architecture be described. Both are the wave-1 `documents` route that `AC-01` and `SA-05` already survived on. The other sixteen are acts. The distinction had to be drawn precisely because two of the eighteen sit one control away from records going the other way — `CP-03` is contingency TRAINING beside `CP-02`'s contingency PLAN, and `PS-09` asks that an existing HR artifact incorporate something rather than that the system produce one — and a pass reasoning from the family name rather than from the verb would have adjudicated each pair identically. That is precisely how risk 3 produces a wall of assertions, and both boundary records argue the line rather than assert it.

**Neither partial adds coverage and both add work.** `recipeIds` is empty on each: N0 decision 3 says a partial never discharges, so `reachable` moves 36 → 38 and `discharged` stays at zero, while the unlinked authoring backlog goes **13 → 15**. Each needs its own `kind` — a `contingency-plan` and a `security-architecture` — because riding `system-documentation`, whose subject is SA-05's administrator, user and operations documentation, is exactly the generic-`policy` over-claim the closed enum was written against.

**`remainderAnsweredBy` was available for none of the eighteen, and that is the batch's contribution to N1d.** The pointer built in N1b′ resolves against upstream's published evidence plan, and a validated walk of it — 234 items, 96 controls proved, sanity-checked against `sa-10`, `cm-4.2` and `si-7.7` — finds **no upstream recipe over any of the eighteen on either plane**. All eighteen rows read `coveredBy: []`. So these controls are refused here and unanswered anywhere either project has published, which is a different and harder statement than the ceiling has been able to make so far: not "the commit plane cannot reach this", but "nobody has reached this yet."

**The first batch whose divergences run both ways, and three of the five run against us.** Until now every divergence in this overlay was this plane reaching further than upstream, and the pinned list in `frontier.test.ts` could be read as a tally of wins. `CP-02` and `PL-08` are the familiar direction. `CP-04`, `MA-02` and `PS-05` are the other one: upstream reaches `partial` on all three and this plane reaches nothing, because all three turn on an **execution record** — a restore test that ran, a maintenance window that ran, a group membership that changed — and an execution record is the class of evidence a checkout structurally cannot hold. They are filed as divergences rather than quietly as agreements because a reader comparing the two overlays on those controls should be sent to upstream's pass, not reassured by a second refusal. The test's literal grew by an edit someone had to make, which its own comment says is the only way it should grow.

#### The auditor gate, second run — twelve findings, all real, and one of them a relapse

Every finding was verified against the code before being applied; none changed a disposition, and all twelve changed the text.

**The most serious was a regression of a correction the previous audit had already made.** The `PL-10` record's centrepiece was that `rampscan.config.json` declares a certification class, refused as evidence because a scanner's own setting must not discharge a control about the system's authorization. The argument was good and **the artifact does not exist**: the config declares `graph`, `contract` and `documents` and nothing else, and the class is a command-line flag read per invocation and defaulting to `b`. The `CA-02` record says so in almost those words, and says so *because the batch-1 audit caught that record missing it*. So a fact an independent pass had taken out of the overlay was written back into it three weeks later by a pass that reasoned from memory of the product rather than from `main.ts`. The correction makes the refusal stronger: there is no committed statement of the claimed baseline anywhere, and what *is* committed — the catalog, the contract and documents blocks — is the assessed configuration, which is the tool reporting its own inputs back as a decision about the system.

**The one genuine over-claim was `CP-02` selling a content hash as a control limb.** The record argued that CP-02 requires the plan be protected from unauthorized modification and that content-anchoring is that limb. Three things are wrong with it and the third is the one that generalises. CP-02 (h) reads "protect the contingency plan from unauthorized **disclosure and** modification", and the record quoted the half that flattered the route while the omitted half runs *against* it — the proposed discharge is committing the plan into the repository, which widens the set of readers. A hash detects modification rather than restricting it. And **no assertion can read it**: the collector emits `present`, `bytes` and `non_empty`, and the content hash rides the bundle as an *anchor*, not as an observation field. `AC-01` and `SA-05` survived this route twice without ever making that jump — both describe anchoring as a property of the evidence — so it was the increment over prior art that failed, not the route. The disposition stands; the sentence is gone and CP-02 (h), (a)(7) and (g) are named in the remainder.

**The remaining ten, in one line each.** `MA-02` cited RA-05 and SI-02 as "already adjudicated in this overlay" when neither is even on the frontier — the artifact is claimed in the *catalog*, by `dependency-update-automation` under SI-02 and SI-05, which makes the double-claiming point stronger than the wrong citation did. `PS-07` claimed the collector set enumerates every component's licence and supplier; `syft.ts` emits `{format, spec_version, component_count, generated_at}` and nothing else, and the licence recipe is on the not-built list. `AT-02 (02)` carried a sentence that, read literally, refuted both partials in its own batch. `PS-05` miscounted its own citation and contradicted itself in consecutive sentences. `PS-02`, `PS-03` and `PS-08` each claimed a different superlative for the same territory, which is escalating rhetoric standing in for distinct reasoning — the batch's stated risk showing through. `PS-08` attributed a `remainder` to a recipe, which recipes do not have. `PS-02` said eight words were nine. `PL-08` inherited an imprecise CM-06 citation from `SA-03`.

**And the capability claim that was over-broad in five records at once.** "Nothing in the collector set reads git history metadata" is the sentence three records already carry, and it is false as stated: `gitleaks.ts` runs `gitleaks git` over the **full committed history**, and its rows carry the commit a leak sits at. The substance survives — nothing dates a path, enumerates its edits or resolves reviewer identity, which is what `AC-01`, `RA-05 (11)` and `SA-05` were cut for — but the wording contradicted `SC-12` and `SR-10` in the same overlay. All five of this batch's uses now read that no collector *emits git-history metadata any assertion can read*, with gitleaks named. `PS-06` needed it most, since its subject is commit trailers and gitleaks' scan surface includes commit messages.

**Flagged and deliberately not fixed:** `ac-1`, `ra-5.11` and `sa-5` carry the same over-broad phrase. Each is scoped by a qualifying clause in the same sentence and each was written and audited under batch 1, so rewriting them is a pass of its own rather than a side effect of this one — the same reason the pre-batch commit took `recipes/pipeline/` separately instead of inside the batch that found it.

**What the command reads now.**

```
frontier 112 uncovered controls · dataset 2026.07.14.01
  adjudicated  0 automatable · 20 partial · 24 narrative
  unreviewed   68        (was 86)
  discharged   0
  catalog covers   23 of 209 controls a KSI reaches   (unchanged)
  reachable        38 of 209 — 18.2%                  (was 36, 17.2%)
```

**The class-b lead set is closed: 0 of 43 unadjudicated**, which is decision 4's whole scope and the end of N1a-T3's near-term target. What remains on the frontier is 68 unreviewed, none of them class b. **695 vitest** green, root and console typecheck clean, `rampscan frontier` exits 0 with no link-check problems.
