...
**Status:** committed plan for the work after Phase H (engine expansion, complete) and Phase I3 (auditor lens, complete — I3e landed 2026-08-15, I3f folded into and landed with J5). **Phases J and K1 are complete**: J1–J5 and K1 all landed 2026-08-15. Everything committed in *this* plan is built. Work has since moved to a **new plan of record**, `docs/IMPLEMENTATION-PLAN-ONTOLOGY-ONDEVICE-LLM.md` (phases L0–L5: the architecture contract, the repo-model artifact, the MCP agent surface, the on-device model port, the narrative evidence tier) — L0 and L1 landed 2026-08-15, L2 and L3a–L3c on 2026-08-16 (L3 was revised before code: the dry run ships as `rampscan check`, and the MCP server — L3d — is deferred until an external user asks, six of its seven tools being re-exports of the JSON surfaces that now exist). Note the letter collision: **"Phase L" in the Tier-2 table below means DAST/ZAP** and is unrelated to the L0–L5 sequence in that document; the DAST decision remains held. The would-be K2 (BYOK AI helper) is superseded rather than merely held — the new plan's L4 answers the same need with a **local** model runtime instead of a cloud key. This document owns **scope and task breakdown** for everything not yet built; `docs/PLAN-OF-ACTION.md` and `docs/PLAN-OF-ACTION-CONSOLE.md` own **task status** and get ticked as work lands. On any scope dispute this document wins over the checklists; on any architecture dispute `docs/SPEC.md` wins over this document.
**Date:** 2026-08-15
...
|---|---|---|
| ~~K2 — BYOK AI helper (T1 "Explain this" first)~~ | BRAINSTORM §3 | **Superseded 2026-08-15**, not merely held: the new plan's L4 takes the same need — a model that drafts prose a human signs — to a *local* runtime with pinned weights, honestly absent when the machine has none. A cloud key was the part that needed a go/no-go, and this answers it by not needing one. |
