...
- **2026-09-18** — R2 unpaused by the owner, decoupled from the S3 exit. The build is broken down in `docs/PLAN-SDR.md`. Upstream facts checked that day: the SDR schema is still `1.1.1`. The second §2.6 divergence (no slot for KMT metrics) was already filed by someone else as FedRAMP/schemas#10, so R2 files only the first (`keySecurityIndicators` missing from `required`) and comments on #10. FedRAMP answered two questions in its issues. On #21, a declined MAY rule is `Not Implemented`. On #19, per-method data may ride in optional extension arrays. The exit gate's "two upstream findings filed" therefore becomes one filing plus one comment, still waiting on the owner's go with the S3 sends.
