...
- **2026-09-18 (R3.1–R3.3 built)** — Stacked as #230 → #231 → R3.3. R3.3 wires the block into `rampscan sdr`. The day folds use the same fold options as the record's own projection, so the history is computed by the same rule as the current row. The metrics are filtered to the record's KSI rows, so the block and the rows cannot disagree. `conformance` stays `unmet` for `SDR-CSX-KMT` and now counts what is carried. Its first real catch was in a test: a class-c record built with class b's optional set was missing 5 obliged KSIs, and the verdict named them. On this repository at class c: 365 days in reach, covered from 2026-08-13 (36 days), 3.5 s end to end. The JSON grew from 99 KB to 256 KB (pretty-printed); most of the difference is the per-count ranges repeated in 46 × 2 summaries, not the runs.
