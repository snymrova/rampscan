...
- [x] **P3-3. Status badges** in the README, added only after both jobs have gone green on the remote at least once. A badge added before the first run is a claim with no evidence behind it. *Closed 2026-08-19: both badges point at workflows green on the remote (b4c0726).*
- [x] **P3-4. Budget the first real run.** This workflow has **never executed** — there is no remote. Expect one to two iterations on tool installs, Docker availability in Actions, and Playwright deps. This is a scheduled cost, not a surprise.
...
- [x] **P4-5. Console screenshots** — the coverage board, an evidence detail page, the drift view. The visual loop is a large share of what was built and is currently invisible to anyone who does not clone and run it.
...
- [ ] **P5-0. The three placeholders that must be closed before the push, not after.** Each is a file that is already correct except for a fact P0-6 has not decided, and each fails silently rather than loudly. (a) ~~`NOTICE` states that `docs/context/ramprules/` is CC BY 4.0 and the sibling carries no `LICENSE`~~ **Closed 2026-08-18: published as `data/LICENSE` on `snymrova/ramprules.com`, CC BY 4.0.** Landed as a single licence-only commit on top of `origin/main` rather than by pushing that repository's 27 unpushed commits, because the blocker was one file and the rest was not ours to publish. Closing it changed `NOTICE` in the opposite direction from the expected one — upstream scoped its grant to `data/derived/` and `data/overlays/`, so the five non-data files in our snapshot are **not** covered and `NOTICE` now says so. (b) ~~`.github/ISSUE_TEMPLATE/config.yml` carries `OWNER/REPO` in two absolute URLs~~ **Closed 2026-08-18: `snymrova/rampscan`.** The hard-coding is inherent — GitHub's issue-form schema takes no relative link — so the comment now says which two lines break on a rename rather than which two are unfinished. (d) **This document.** `docs/PLAN-OF-ACTION-LAUNCH.md` is the last of the ten-plus-one removed under P0-3, and it goes when the phases above it are all ticked — not before, because it is what is being executed. (c) ~~`CODEOWNERS` is unwritten because it needs a GitHub handle.~~ **Closed 2026-08-18** — the handle is `@snymrova`, read off the sibling ramprules remote rather than guessed, and it is the same person who authors every commit here. It is one line to change if rampscan lands under an organisation instead.
- [x] **P5-1. Create the remote** (public, per P0-6) and push `main`. *Closed 2026-08-19: `snymrova/rampscan`, pushed 2026-08-18.*
- [x] **P5-2. Watch the first CI run** and fix forward per P3-4. *Closed 2026-08-19: one fix-forward (`pnpm doctor` → `pnpm run doctor`, b9335bc) and one smoke re-run after a Playwright download ate the 30-minute clock; green since.*
- [x] **P5-3. Handle secret-scanning.** Push protection may still flag the fixture's remaining intentional plants even after P1. If it blocks, the resolution is the allowlist and a dismissal with a reason — never a quiet weakening of the fixture, which is load-bearing for the product's own tests. *Closed 2026-08-19: nothing blocked — the P1 rewrite left nothing for push protection to flag.*
- [x] **P5-4. Tag `v0.1.0-beta`** with release notes assembled from what the phases actually did, and the coverage figures quoted from the command. *Closed 2026-08-19: prerelease on 306ca63, every figure from a command run the same day.*
- [ ] **P5-5. Repository settings** — description, topics, branch protection requiring both CI jobs, and the security policy linked. *Description, topics and the security policy done 2026-08-19; branch protection (`test` + `console-smoke` required on `main`) still owed — tracked as issue #11.*
...
|---|---|---|
| 2026-08-19 | P5 · roadmap | **The plan moves to GitHub, and P5 all but closes.** The remaining work is now issues under three milestones on `snymrova/rampscan` (launch close-out · P6 gate · post-launch hygiene). The two pipeline docs landed (306ca63) after verification against the pinned dataset caught "121 uncovered controls" stale in four present-tense places — the snapshot's rollup says 209 reached · 97 covered (88 aws · 9 pipeline) · 112 uncovered, and `rampscan frontier` prints 112. `v0.1.0-beta` is tagged and released on that commit with every figure from a same-day command run; the draft notes named `doctor` and `export` as CLI commands and the CLI's own dispatch corrected them to the real eleven. Description, topics and the security policy are set; **branch protection is the one P5-5 item still owed** (issue #11), which is also what holds P5-0(d) — this document goes when it lands. Self-scan unmoved: **12 evidenced · 2 violated · 6 unevidenced**. |
| 2026-08-18 | P4-5 · P5-0(a) · CI | **P4 closes, P5-0(a) closes, and two defects were found by doing rather than by planning.** P4-5's first capture was discarded: serving the working ledger put an unrelated project of the author's on the board, so the three shots come from a purpose-built projection (fresh ledger/keys/PocketBase, fixture scanned twice with `CODEOWNERS` added between) — which is also what makes the drift page show a real `violated → evidenced (anchor-drift)` flip attributed to its commit. P5-0(a) closed by publishing `data/LICENSE` to `snymrova/ramprules.com` as a licence-only commit, and closing it made `NOTICE` **narrower**: upstream scoped CC BY 4.0 to its data directories, so the five non-data files in our snapshot are named as ungranted rather than swept into the claim. **CI: `testTimeout` 5s → 30s.** The slowest test measures 5115ms warm on this machine — already past the default — so `test.yml` would have failed its first run on the clock and read as a misconfiguration. **And the fixture builder could commit into this repository**, which it did once during this session: git discovery walks up when `root/.git` is missing, finds the repo the fixtures live inside, and `git add -A` writes the working tree into history under `fixture@rampscan.invalid`. Removed before any push, `GIT_DIR`/`GIT_WORK_TREE` now pinned in both builders, one-identity invariant verified green. Self-scan unmoved at **12 evidenced · 2 violated · 6 unevidenced**. |
