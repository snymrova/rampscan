...
- [x] **S1-4. Configured entry points stop hiding detection.** `detectEntrypoints` still honours config, and additionally reports what package.json detection *would* have found and config excluded, so narrowing is visible instead of silent. *Done 2026-09-14 (extractor 0.5.0):* detection runs over every S1-3 application root rather than the tree root alone — a monorepo's root manifest declares nothing, which is why detection used to find nothing and config was the only way to start a walk. It reads each root's `main`/`module`/`bin`/`exports`, the source files named on its `scripts` command lines (this repository runs its CLI as `"rampscan": "tsx packages/cli/src/main.ts"` and declares no bin), and two framework conventions no manifest field could name: Next.js (`app/**/{page,layout,route,…}`, `pages/**`, `middleware`, `instrumentation` under a `next.config.*`) and PocketBase (`pb_hooks/*.pb.js`, `pb_migrations/*.js`). Config wins for the walk; `graph.db` records `entrypoints_detected` and `entrypoints_excluded`, the gate's `walkWidth` measures which excluded entries the configured walk reached anyway, the basis signs `entrypoints_excluded` with `reached` per entry, the OpenVEX `rampscan:scope` carries the same, and the console names them. **An excluded entry point the walk never reached refuses the negative** for the run, the same way an unentered root does — a place the program starts that no walk began from or arrived at is ground rule 2's absence-of-evidence, and a config is honoured as an instruction, not as a proof. A config-sourced graph that predates the record has an unknown narrowing and refuses too. **Measured on this repository:** the config's one entry point starts a walk that reaches 109 of 285 files and 12 packages; detection finds **57** entry points (10 from manifests, 3 from `scripts` — one of them the config's own `main.ts` — 24 Next.js, 20 PocketBase) and a walk from them reaches **184** files and **15** packages — `next`, `react` and `pocketbase` join the reached set — and enters **every one of the twelve roots**, `console/web` 45 of 46 and the workspace root 23 of 33. Under the config as it stands, 56 detected entry points are excluded and 47 never reached, so the self-scan's negatives stay refused with that as the recorded reason. S1-5 is therefore a one-line change: drop `graph.entrypoints` from `rampscan.config.json` and let detection start the walk.
- [ ] **S1-5. Regenerate, and let the board move.** Re-run the self-scan, with `graph.entrypoints` removed from `rampscan.config.json` so detection (S1-4) starts the walk from all 57 entry points and every root is entered. Expected: `postcss` resolves `reachable: true` via `next` — **which requires S1-4 first**, because `next` is reached only through `console/web`, and the configured entry point set does not include it (S1-2 measured this; finding §4.4 corrected); `sharp` resolves `unknown`; `openvex.json` carries **zero** `not_affected` statements; and `no-critical-reachable-advisories` flips **`evidenced` → `violated`**, because two `HIGH` advisories are in fact reachable. The self-scan line moves from `12 evidenced · 2 violated` to `11 evidenced · 3 violated`. **This is the fix working, and the README must say so** — a third standing violation, explained, is worth more than the suppression it replaces. *Done 2026-09-14, at `c0d1c73`:* `graph.entrypoints` deleted from `rampscan.config.json` (five lines, nothing else touched — the config is signed into every basis); `rampscan scan .` run at that commit with every tool resolving. **The board moved: `12 evidenced · 2 violated` → `11 evidenced · 3 violated`**, `no-critical-reachable-advisories` `evidenced` → `violated`, and the README's "It scans itself" now leads with it. What the row counts is wider than predicted, because the advisory set is September's, not August's: `next@15.5.23` carries two advisories (9.5, 9.0) that are reachable by an **exact one-hop path** from every console route (`console/web/app/api/artifact/route.ts » next/server`); `postcss@8.4.31`'s two HIGH advisories are reachable through the SBOM's `next → postcss` edge with the hop marked `sbom`, as predicted; `sharp@0.34.5`'s two HIGH advisories read `unknown`, as predicted, and count. Six rows, `expected count == 0, got 6`. `openvex.json` carries six `affected`, three `under_investigation`, and **one `not_affected` the plan did not predict**: `vitest@4.1.10` against GHSA-82fw-gwwq-j7x9 (5.9, published after this plan was written). It is the earned shape and nothing else — `vitest` has a node (the test files import it), the walk started from all 57 detected entry points, entered all 12 roots, excluded nothing, and never arrived; `rampscan:scope` on the statement says so in fields. `rampscan rebuild`: projection ≡ ledger. `rampscan report` regenerated `docs/FRONTIER-PIPELINE.md`, whose committed copy had been from a run on 2026-08-13 — the class of drift S2 exists to gate. Suite green.
...
> **Exit gate read 2026-09-14, at `c0d1c73`:** S0-3 passes. `postcss` carries `… » next » postcss` with the last hop `sbom`; `sharp` reads `unknown`. Three violations; the README leads with the third. `rebuild` byte-equal. Suite 1,150 green. **One bullet is not met on its letter:** `openvex.json` carries one `not_affected`, for `vitest@4.1.10` — an advisory that did not exist when the bullet was written, against the one package in the set that has a graph node and is genuinely outside every walk. The bullet's intent was *no vacuous negative*, and the statement is the opposite of vacuous: it names 57 entry points, 12 roots all walked, and no exclusion. Recorded as met in intent and missed in letter, rather than the bullet quietly reworded; ground rule 3 applies to this document too.
...
- [x] **S3-0. The ingest adapter stops reading an exit code as a verdict (#147).** Added 2026-09-15, ahead of S2, because S3-1 is the first time a real tree of this shape is ingested and the adapter would have signed `evidenced` over a violating account. `docs/RESEARCH-PARAMIFY-PILOT.md` §8.1: the scripts the tree convention was read from exit 0 after *reading* the account — `s3_encryption_status.sh` at 0 % encrypted buckets, `guard_duty.sh` on "No GuardDuty detectors configured" — and `ingest.ts:157` wrote `passed: exit_code === 0` as the single assertion. The `SECURITY.md` class, same as GHSA-7jff-6v53-r56x, different input; disclosed in the open under S0-1's posture (the path had only ever run on the fixture). *Done 2026-09-15, the S0-3 pattern in one PR:* `packages/cli/test/ingest-soundness.test.ts` written first and run at `f4c27bd` — `expected 'evidenced' not to be 'evidenced'` on an exit-0 tree whose rows hold a NONCOMPLIANT principal. Then the fix: a manifest entry may declare `assertions` in `aws-evidence.json`'s vocabulary, and the appliance evaluates them over the result file's rows with the pipeline's own evaluator (`evaluateAssertions`, offenders and `population` included — the evaluator the cloud runner's T2-3 will share); an exit-0 entry declaring none is signed **`unevidenced`**, a collected artifact and not a pass, which the contract now permits (`assertions` may be empty; `submissionVerdict` gained the third arm and lost its vacuous one); a non-zero exit is a **failed run**, skipped and named in the outcome and the log like a skipped collector, never a bundle under any verdict. The fixture's three entries each carry one `status eq` assertion; `KSI-IAM-AAM` reads `violated` because the appliance found its NONCOMPLIANT row, where before a script said 3. SPEC §12.8 rewritten to say what the exit code means and why. **Not done here, deliberately:** routing a collected-only bundle into the two-key sufficiency judgment (Q3.3) is T0-2's decision and the runner plan's to build; until then `unevidenced` is exactly what such a bundle is.
...
- **2026-09-14 (barrel edge, S4-1 pulled forward)** — `export … from "x"` is an `imports` edge (extractor 0.4.0). Nine lines in `extract.ts`: the `ExportDeclaration` branch now calls the same `importTarget`/`importEdge` the `ImportDeclaration` branch does when a module specifier is present, so a barrel is a door and not a wall. Tested on a planted `@acme/barrel` package whose `index.ts` is nothing but the four re-export forms — each an `exact` edge, the walk reaches every re-exported file and still not the orphan. Measured on this repository before merging: files reached 60 → 109; every `@rampscan/*` package's `src/` fully entered but one `collectors` file and seven `cli` files; dependency packages reached 12 → 12, so nothing on the self-scan's board moves — every package behind a barrel here was also imported by a file already reached. That is the honest reading: the fix closes a false-negative shape this tree happened not to pay for, and would have on any single-application repository built of barrels. Suite 1,140 passed, 91 files. Next: S1-4 (#131).
- **2026-09-14 (S1-4)** — Detection stops being something config switches off. It runs over every application root, reads manifests, `scripts` command lines, and the Next.js and PocketBase file conventions, and what config left out travels with the graph into the basis, the VEX scope and the console — with `reached` per entry, because an excluded entry the configured walk reaches anyway narrowed nothing. One judgment made and recorded rather than deferred: an excluded entry point the walk never reached refuses the negative, like an unentered root. Measured on this repository: config 1 entry point, 109 files, 12 packages, two roots never entered; detection 57 entry points, 184 files, 15 packages, every root entered. The path to S1-5 is now a deletion in `rampscan.config.json`. Extractor 0.5.0, reachability 0.4.0, SAST gate 0.3.0. Suite 1,150 passed, 91 files. Next: S1-5 (#132).
- **2026-09-14 (S1-5)** — The override came out of the config and the board moved: `12 evidenced · 2 violated` → `11 evidenced · 3 violated`. Wider than predicted, because September's advisory set is not August's: `next` itself carries two CRITICAL advisories reachable by an exact hop from every console route, beside the predicted `postcss` via `sbom` and `sharp` as `unknown`. One `not_affected` the plan did not predict, `vitest@4.1.10`, and it is the earned shape — node present, 57 entry points, 12 roots walked, nothing excluded — so the exit gate's "no `not_affected`" bullet is recorded as met in intent and missed in letter. `docs/FRONTIER-PIPELINE.md` regenerated from the run (its committed copy was a month old). **S1 closes.** Next: #147 before S3-1, then S2 (#133–#136).
- **2026-09-15 (S3-0, #147)** — S1 closed at `286c566` (#155 merged; the earned `vitest` negative stands as recorded in the exit-gate reading). #147 taken next, out of phase order, because it is the `SECURITY.md` class and S3-1 is where it would have fired. The failing test first, at `f4c27bd`: an exit-0 tree over rows holding a NONCOMPLIANT principal signed `evidenced`. The fix reads the exit code as the scripts mean it — 0 finished reading, non-zero could not — and takes the verdict from structured assertions the appliance evaluates itself, with the pipeline's evaluator and `aws-evidence.json`'s vocabulary, so the tree adapter and the future runner share one evaluator; no assertion is `unevidenced`, a failed run is skipped and named. The contract permits empty `assertions` now and `submissionVerdict` no longer has a vacuous branch. The fixture's `KSI-IAM-AAM` still reads `violated` on the board, for a reason that is now true. Next: S2 (#133–#136).
