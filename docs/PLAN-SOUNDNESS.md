...
**Status:** proposed plan of record for one deliverable — *every claim rampscan publishes is one a stranger can check, and one stranger has checked*. This document decides nothing until it is merged and its milestones exist on GitHub; that is the adoption bar the KSI pivot and the artifact plane both cleared.
**Date:** 2026-09-13
...
There are zero external adopters (§1), so nobody holds a false VEX document from this tool today. That bounds the blast radius; it does not change the class. The artifact is committed, published, reachable on GitHub, and is the file a reader most likely to matter — someone evaluating whether signed reachability evidence is real — would open first.
...
- [ ] **S0-1. Disclosure posture.** *Owner's decision.* Recommendation: **advisory filed, no embargo, fix in the open.** `SECURITY.md` directs this class to GitHub private vulnerability reporting, and following your own policy for a finding you made yourself is the cheap way to prove the policy is real. With 0 stars, 0 forks and 2 views in fourteen days (`gh api …/traffic/views`) there is no adopter to protect and therefore no argument for an embargo. A published advisory that *names the tool's own defect in its own signed output* is, for a compliance-evidence product, an asset rather than a liability — it is the single most credible thing this repository could contain.
- [ ] **S0-2. What happens to the committed `openvex.json`.** *Owner's decision.* Recommendation: **supersede, never delete.** The document is wrong and it is also the evidence that the defect was real. Deleting it is the one move inconsistent with everything the ledger stands for. Regenerate it post-fix in the same commit and let the diff be the proof.
- [ ] **S0-3. The failing test, first.** A test asserting that a package present in the SBOM and absent from `graph.db` does **not** produce a `not_affected` statement. It must fail at `764a819`. Nothing in S1 begins until it does.
- [ ] **S0-4. Record the pre-fix state.** The five statements, the three `HIGH` rows, the 17 graph nodes, the `next → postcss` SBOM edge — each with the command that produced it — as the advisory's reproduction section.

**Exit gate:** the advisory exists; `S0-3` fails, reproducibly, naming `reachability.ts:142`; S0-1 and S0-2 are recorded decisions in this document, not preferences.

### Phase S1 — reachability soundness (2 days)

- [ ] **S1-1. Absence becomes `unknown`.** `dep === undefined` no longer reaches the `notAffected` branch. `not_affected` is emitted **only** where the package has a node and the walk did not arrive. Ground rule 2, in one edit.
- [ ] **S1-2. The SBOM graph joins the walk.** `dependencyReachability` takes the CycloneDX `dependencies[].dependsOn` edges and continues through them from any reachable dependency node, so a transitive dependency of a reachable direct dependency resolves `reachable: true` with the package chain as its path. Hops sourced from the SBOM are marked `sbom`, distinct from `exact` and `inferred`, because their provenance is a package manifest rather than a parsed call site and a reader is owed the difference. **This upgrades `unknown` to `true`; it may never downgrade anything to `not_affected`** — the SBOM graph is partial by measurement (34 of 173 components carry edges), so it can prove presence and never absence.
- [ ] **S1-3. A negative claim states its scope.** The VEX statement carries the entry-point set and the walked application roots as structured fields, not only inside prose. Where the tree contains an application root that no declared entry point covers, `not_affected` is **refused** for that run and the reason is recorded — a negative claim scoped to half a repository is not a negative claim.
