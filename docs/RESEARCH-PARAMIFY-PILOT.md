...
| The assessed package YAML as an ingest input | **Done 2026-09-15 (S3-1).** `rampscan ingest <package.yaml>` — one `IngestSubmission` per (KSI × evidence), `assessedBy` under `assessorOrg` as `signer_identity`, `point-in-time`, artifacts by `reference` beside the package file's own digest. One deviation from the row as first written: `assessmentStatus` is **not** the assertion. It is a person's reading, and #147's rule holds — the appliance signs only what it evaluated — so every bundle is `unevidenced` and `automated: false`. The register it produces is `docs/RESEARCH-PARAMIFY-REGISTER.md` | done |
...
- **2026-09-15 (S3-1)** — The package adapter, and the register over the 8/29 package (`docs/RESEARCH-PARAMIFY-REGISTER.md`). §8.3's package row closed with one correction: `assessmentStatus` is not signed as an assertion. The crosswalk §4 budgeted for exists (`recipes/crosswalks/ksi-phase-one-to-2026.07.14.01.json`): 43 of 51 indicators placed, 8 retired to FRR families, 3 obliged 2026 KSIs the package never reaches.
