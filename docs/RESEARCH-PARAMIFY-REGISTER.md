...
**Input:** `paramify/fedramp-20x-pilot` at `51de49c`, `machine-readable-package/8_29_25_paramify_coalfire_20x_machine_readable.yaml` — the FedRAMP 20x Phase One package Paramify submitted and Coalfire assessed (`recommendation: Authorize`, `date: 7/10/25` as the file states it). One of **two** publicly assessed, 3PAO-signed Phase One packages this project knows of — the other is Filevine's (`Filevine/fedramp20x-low-submission`, same 3PAO and lead assessor, exported from Paramify), read in §3b; `docs/RESEARCH-PARAMIFY-PILOT.md` §6 said "only" and was corrected 2026-09-15.
...
## 3b. The second public package: Filevine, same shape, same register

`Filevine/fedramp20x-low-submission` (Phase One Low, Vinesign in scope; last push 2025-08-19; no LICENSE, same posture) ships `machine-readable-package/filevine-coalfire-2025-assessment.yaml`: `assessorOrg: Coalfire`, `leadAssessor` the same as Paramify's, `date: 2025-08-18`, and a signed Validated Assessment Letter beside it. Its README says the package was generated by Paramify's export and validated in Paramify by the 3PAO. On the file: 51 validations, **all 51 `assessmentStatus: "False"`**, `recommendation: ""`; 237 evidences, **0** marked `automated: Yes`; 708 artifacts named; `validationRules: []` on every evidence; every per-validation signature `""`. The `False` beside a signed letter is almost certainly an export with the assessment fields unpopulated — and it is upstream's own data making §1's point: the machine-readable file and the human reading disagree, and signing neither as a verdict is what keeps a ledger honest over it.

Ingested unchanged — same crosswalk, `--cadence monthly` declared (to be confirmed from their trust center as Paramify's was), `--repo filevine-for-government`:
...
157 methods on 38 KSIs; 73 skipped under the eight retired indicators (13 of them `*-pol-and-proc` policy documents under PIY-02 alone). The register, package alone, class b:

| KSI | methods | freshest | worst gap |
|---|---|---|---|
| KSI-CED-RAT | 11 | 393d | G2 |
| KSI-CMT-LMC | 2 | 405d | G2 |
| KSI-CMT-RMV | 4 | 396d | G2 |
| KSI-CMT-RVP | 8 | 396d | G2 |
| KSI-CMT-VTD | 2 | 405d | G2 |
| KSI-CNA-DFP | 3 | 396d | G2 |
| KSI-CNA-EIS | 0 | — | G1 (optional at class b) |
| KSI-CNA-IBP | 2 | 396d | G2 |
| KSI-CNA-MAT | 4 | 396d | G2 |
| KSI-CNA-OFA | 3 | 396d | G2 |
| KSI-CNA-RNT | 1 | 404d | G2 |
| KSI-CNA-RVP | 1 | 403d | G2 |
| KSI-CNA-ULN | 3 | 396d | G2 |
| KSI-IAM-AAM | 0 | — | G1 |
| KSI-IAM-APM | 1 | 403d | G2 |
| KSI-IAM-ELP | 8 | 396d | G2 |
| KSI-IAM-JIT | 6 | 403d | G2 |
| KSI-IAM-SNU | 9 | 396d | G2 |
| KSI-IAM-SUS | 1 | 400d | G2 |
| KSI-INR-AAR | 6 | 396d | G2 |
| KSI-INR-RIR | 0 | — | G1 |
| KSI-INR-RPI | 9 | 393d | G2 |
| KSI-MLA-ALA | 0 | — | G1 (optional at class b) |
| KSI-MLA-EVC | 1 | 405d | G2 |
| KSI-MLA-LET | 0 | — | G1 |
| KSI-MLA-OSM | 1 | 393d | G2 |
| KSI-MLA-RVL | 3 | 400d | G2 |
| KSI-PIY-GIV | 4 | 403d | G2 |
| KSI-PIY-RES | 2 | 404d | G2 |
| KSI-PIY-RIS | 2 | 404d | G2 |
| KSI-PIY-RSD | 9 | 396d | G2 |
| KSI-PIY-RVD | 2 | 405d | G2 |
| KSI-RPL-ABO | 2 | 403d | G2 |
| KSI-RPL-ARP | 7 | 396d | G2 |
| KSI-RPL-RRO | 3 | 396d | G2 |
| KSI-RPL-TRC | 4 | 396d | G2 |
| KSI-SCR-MIT | 17 | 393d | G2 |
| KSI-SCR-MON | 3 | 393d | G2 |
| KSI-SVC-ACM | 4 | 400d | G2 |
| KSI-SVC-ASM | 1 | 403d | G2 |
| KSI-SVC-EIS | 3 | 404d | G2 |
| KSI-SVC-PRR | 0 | — | G1 (optional at class b) |
| KSI-SVC-RUD | 0 | — | G1 (optional at class b) |
| KSI-SVC-SIN | 3 | 396d | G2 |
| KSI-SVC-VCM | 0 | — | G1 (optional at class b) |
| KSI-SVC-VRI | 2 | 400d | G2 |
...
The same three KSIs no method reaches — **`KSI-IAM-AAM`, `KSI-INR-RIR`, `KSI-MLA-LET`** — and that is not a fact about either CSP: no entry of the crosswalk names any of the three, because no Phase One indicator's subject lands on them. They are new at 2026, and a Phase One validations set cannot reach them by construction. Freshest method 393 days old; every row `G2 methods`.

Two CSPs, two impact levels, one 3PAO, one exporting tool, and one register. What it measures is the Phase One package format under the 2026 rules, not a package.
...
The question S3-2 asks, unchanged: *is the gap register the thing you are missing, or is the SDR?* The registers above are the first ever computed over real authorized packages — two of them, and the same — and it is what a class-b certification would be measured against on the day the 2026 rules apply.
...
3. ~~**The history meter reads the earliest instant**~~ a KSI's chains reach back to, so one 2025 snapshot satisfied class c's 6-month floor on 38 rows. A single stale capture is not "persistent validation", and this run was the first to make that reading visible; filed as #159 and **fixed 2026-09-16**: the meter now walks the instants and wants the status refreshed inside each owed window, so the §4 what-if at class c reads **history ≥6 mo on 0 of 46**, each row naming the instant its status lapsed (the first 2025 capture plus three months on the non-machine clock). The figure in §4 above is left as it was printed, with this note as its correction.
...
- **2026-09-15 (later)** — §3b added: a second public package exists (Filevine's), the header's "only" corrected, and the same register computed over it; the three unreached KSIs restated as a catalog fact. Found while researching S3-2's channels (`docs/RESEARCH-S3-2-OUTREACH.md`).
