...
3. **The history meter reads the earliest instant** a KSI's chains reach back to, so one 2025 snapshot satisfies class c's 6-month floor on 38 rows. A single stale capture is not "persistent validation", and this run is the first to make that reading visible; filed as a follow-up on the fold's `FRC-CSX-MOT` arm rather than fixed here.
