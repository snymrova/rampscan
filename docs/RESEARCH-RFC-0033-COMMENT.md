...
**Status:** draft, 2026-09-16, at `main` = `09d5b42`; refreshed 2026-09-18 at `eb7257b` against rules 2026.09.13.02 and the thread as it stands. Not filed.
**Where it goes:** [FedRAMP/community #170](https://github.com/FedRAMP/community/discussions/170), the formal comment thread (opened 2026-09-09, closes **2026-10-09**, three comments as of 2026-09-18; FedRAMP does not reply there). Not #172, the informal thread, and not a new thread — `docs/RESEARCH-S3-2-OUTREACH.md` §1 keeps the RFC channel separate from the S3-2 sends.
...
**Reads against:** the RFC body (fetched 2026-09-16 via `gh api`), `docs/context/fedramp-rules/fedramp-consolidated-rules.json` (2026.07.14.01, re-checked at 2026.09.13.02), `docs/RESEARCH-PARAMIFY-REGISTER.md` §3–§6, `docs/RESEARCH-KSI-GAP-ENGINE.md` §2.3, `docs/PLAN-KSI-PIVOT.md` §4 ("the positioning statement, drafted once, used twice"), `docs/RESEARCH-S3-2-OUTREACH.md` §2 (the community answers diagnoses and ignores pitches).

---

## 1. What the RFC asks for, and what this comment answers

The RFC wants "explicit targeted feedback rather than general commentary". Of its five anticipated Class D expectations, two are in this project's evidence: **(4)** "ongoing support for integrating assurance data into agency GRC systems" and **(5)** "new Key Security Indicators related to assurance engineering, automation capabilities". The three comments posted so far are about item (3), the certified-third-party rule (two), and whether a Class D pilot participant can skip a parallel Rev5 Moderate assessment (one, 2026-09-17). Nothing posted yet touches the automation floor that already defines Class D in CR26: `FRC-CSX-VVK` (MUST ≥4 automated methods per KSI) and `FRC-CSX-MOT` (MUST 18 months of persistent-validation history).
...
| Every row above still reads the same at rules 2026.09.13.02; the `FRD` term list still has no *automated* or *method* | re-checked 2026-09-18 |
| Prowler's CR26 20x KSI output has a check for 33 of 46 KSIs on AWS and emits one `MANUAL` row, with no resources, for each of the other 13; an assertion of "no FAIL rows" passes on all 13 | `RESEARCH-PROWLER-INGEST.md` §2d, §4c; P3-2's soundness test |
...
- **Prowler's name.** Point 1 describes it ("the most widely used open-source cloud scanner") without naming it. The observation credits it — it declares its own non-coverage — and the hazard is the consumer's, but naming another project in a regulator's thread reads as a jab. The owner may name it if that reads better.
...
3. **Whether to post before or after the S3-2 sends.** *(Unchanged on 2026-09-18 and now tighter: 21 days remain, so "send, wait seven days, post" still fits only if the sends go this week.)* The comment names both packages and links the register; the S3-2 plan says the CSPs see the register before it is cited anywhere public (§5 step 1). Posting this first breaks that. Either the S3-2 emails go first (a week's wait), or the comment links a register with the package names removed — the numbers survive, the provenance is thinner. Recommendation: send the emails, wait seven days, post; there was time (23 days on 2026-09-16) and the emails are drafted.
...
- **2026-09-18** — Refreshed at `eb7257b`. Every cited fact re-checked against rules 2026.09.13.02: unchanged, and `FRD` still defines no *automated* or *method*. Thread re-read: a third comment (2026-09-17, Rev5 Moderate vs the Class D pilot), still nothing on the automation floor. Added one sentence to point 1 from P3's Prowler research: a scanner's `MANUAL` rows make "no FAIL rows" pass on the 13 KSIs it cannot see. That is the empty-population hazard in shipped tooling, not a hypothetical. Not filed; the §4.3 ordering call is the owner's and the window now requires the sends this week.
