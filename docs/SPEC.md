...
**`rampscan conformance` reads a file, which is the whole point.** `exports` can only ever check the bytes it just produced — the one case where the answer was never in doubt. G10 is "certification JSON not matching FedRAMP schemas", and the document in front of a provider is usually one they already have: written by another tool, edited by hand, or generated by a rampscan that predates the current pins. So the check takes a path (a file, or a directory whose `*.json` documents are all read), resolves which pinned schema gates each one, and validates it. It defaults to `<out>/exports/`.
