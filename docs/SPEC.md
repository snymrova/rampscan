...
**The Prowler adapter** (P3, `packages/cli/src/prowler-ingest.ts`, `docs/RESEARCH-PROWLER-INGEST.md`): `rampscan ingest <file.ocsf.json>` accepts the OCSF compliance output of a Prowler **AWS** scan run with `--compliance fedramp_20x_ksi_2026`, sniffed on content (a bare JSON array, where a native submission is an object) and joined against the vendored framework at its fifth pin. One submission per (Prowler check × KSI), grouping every row that check reported for that indicator — `recipe_id` is the check id, upstream's name. **The appliance evaluates its own assertion** (`count_eq 0` over rows whose effective `status_code` is `FAIL`) with the pipeline's evaluator; Prowler's PASS/FAIL is data, never a verdict signed on its say-so, and `automated` follows the runner's rule (`assertions.length > 0`). Four refusals carry the adapter: a synthetic `MANUAL` row is never evidence — its indicator is skipped and named, never a bundle, and stays in G1; an assertion never passes over zero rows; the exit code decides nothing but whether the run failed (non-zero skips everything, and `--exit-code` is **declared**, because the document records none); and a muted finding is **counted, not waived** — a Prowler mutelist is not a rampscan adjudication. The document carries no header, so the scan's arguments are unrecoverable and a `--status PASS` scan reads like a clean one: every assertion's `detail` therefore states the population as `reported/mapped` AWS checks for the KSI against the pinned framework, and the log prints the same for every KSI the framework maps. A check the framework maps only on another provider, a non-AWS account id, or a `MANUAL` row on an indicator the pin maps AWS checks to refuses the batch. `--signer`, `--exit-code` and `--cadence` are required; none is guessed.
...
### 14.2a The registry (T3-2)

A runner's key enters the appliance the way every other policy fact does: as a two-key ledger statement. `RunnerRegistration` (`https://rampscan.dev/runner-registration/v1`) carries `registered | revoked`, the runner's name (a slug — it becomes `runner:<name>` in a bundle's `signer_identity`), its public key (SPKI PEM, ECDSA P-256), the `keyid` (sha256 over the SPKI DER) its envelopes carry, where it is deployed, the account and partition it is expected to observe, and the proposing and approving identities; the subject is the key by digest. What stands is the fold: the latest statement per runner name, dropped when that is a revocation — an append-only ledger un-decides by deciding again. A transcript is read only when its envelope's payload type is a transcript's (`application/vnd.rampscan.run-transcript+json`, never the in-toto type — a transcript can never parse as a ledger statement), its keyid names a runner that stands, the signature verifies against that key, and the transcript's own `runner.name` is the registered one. `rampscan runner register|revoke|list`; `rampscan verify` renders a registration like a scoping.

### 14.2b The two hosts and the wire (T3-4)

Both hosts are outbound-only clients of two routes the console serves (T4-2). **`poll`**, the sidecar: `POST /api/runs/next` with a signed *claim* (`application/vnd.rampscan.run-claim+json` — `{ runner, requested_at }`, proof of possession of a registered key, fresh); the reply is an assignment (`request`, `request_digest`, the argv the appliance classified) or 204; then identity, the self-check, the run; `POST /api/runs/intake` with `{ envelope, outputs }` — the signed transcript and the raw bytes base64 by digest. **`once`**, the CloudShell one-shot for an account that grants no standing role: the console shows one command carrying a *token* (`https://rampscan.dev/run-token/v1` — the console URL, the nonce, the request digest, an expiry — signed by the appliance's own key and base64url-encoded); the runner fetches the assignment by token, generates an **ephemeral P-256 key**, runs, and posts `{ envelope, outputs, token, public_key }`. Intake verifies its own signature on the token and accepts the ephemeral key for that nonce and nothing else; the bundle's `signer_identity` reads `runner:once-<nonce[0..8]> (<caller ARN>)`, so a one-shot is never mistaken for a registered sidecar. Registration's two-key write is bypassed for `once` on purpose: the token is single-use and minted by the click, which is single-key because it requests a read; the STS identity in the transcript is what the assessor pulls on.

### 14.2c The ledger's record of a run (T4)

Two more statements, both signed by the appliance's key and both single-key because they record facts, not decisions. **`RunRequestEvent`** (`https://rampscan.dev/run-request-event/v1`) is the click: the `RunRequest` exactly as the runner receives it, its digest, the argv the appliance classified (bound — a stranger's account id never rides here), per-step transform and label, the cadence; the subject is the request by digest. **`RunEvent`** (`https://rampscan.dev/run-event/v1`) is each state change by nonce — `claimed` (by which runner), `submitted`, `accepted` (naming the evidence bundle's digest), `refused` (the reason), `failed` (the class) — and `expired` is read from the clock by the fold, never appended. The Runs page's cloud section is `runsLifecycle` over these; nothing writes a row. The routes: `GET/POST /api/runs/request` (a session — any signed-in user; list, then mint), `POST /api/runs/next` and `POST /api/runs/intake` (no session — the runner's signature, or the appliance's on a token), `POST /api/runs/token` (a session; the `once` paste), `GET /api/runs` (a session; the fold). `rampscan serve --repo <path>` names the repository a run is requested for: its `aws` block is the account, and its resolved path is the `repo` these statements carry, so the evidence joins the same register the scan of that path does.
...
The pipeline's evaluator reads **row-wise** assertions: `field` names a column of every row the collector emitted, `where` filters rows, `population` is the row count. That is the tree adapter's vocabulary. Upstream's AWS plane writes a second one: `field` is **`<label>.<JMESPath>`** — `mfa-enabled-for-iam-console-access.EvaluationResults count_eq 0`, `describe-organization.Organization.FeatureSet eq ALL`, `list-roots.Roots[0].PolicyTypes[?Type=='SERVICE_CONTROL_POLICY'] | [0].Status exists` — where the label names one step of the recipe and the path is evaluated over that step's whole JSON document. At the 36 assertion-bearing recipes of overlay 4.3.0 it is **every one of the 188 assertions**, and the label is **upstream's own published step name**: a `collection.commands` entry is a `{name, run}` pair, and `name` is what the assertion addresses. Three addressing forms, all published: `<label>.<path>`, `<label>[<path>` (the path opens on a projection — `credential-report[].mfa_active`) and `<label>` alone, which is the document itself.

Nothing in this repository keys a step by its POSITION. It did until the 2026.09.13.02 re-pin, through a rule (the step's `--config-rule-name`, else its CLI operation) plus `recipes/aws-actions/labels.json`, a reviewed table of the five names upstream had chosen by hand and the rule could not derive. Both are gone: upstream renumbers freely — the credential report went from two steps to three inside one overlay series — and a table keyed to an index is one inserted step away from labelling the wrong output while still looking current. The name travels from the overlay through `RecipeStep.label` into the request's `labels` and into intake, and `aws-labels.test.ts` is golden that every assertion of every pinned recipe addresses a step that recipe publishes and that no two steps of one recipe share a name. `derivedStepLabel` survives for one case only: a hand-built transcript that arrives with no labels at all.

Intake keeps every step's parsed output under its label — steps sharing a label merge their documents, objects shallow-merged and arrays concatenated — as well as the union of its collection rows, and evaluates each assertion in the vocabulary its `field` is in (`packages/core/src/assert-labeled.ts`, since T2-5): resolve the label, evaluate the JMESPath over that document, apply the op to what it yields — `count_eq`/`count_lte` over the length, `eq`/`in`/`lte`/`gte`/`max_age_days` over a scalar or every element, `exists`/`not_exists` over presence — with `population` the element count and **nothing vacuous**: `eq` over an empty projection fails, a label the run did not produce fails naming it, a path that does not parse fails naming the recipe's own defect. A failing element that names a resource under AWS's keys becomes an offender pointer with `resource_id`, `resource_type`, `region`. A `--query X --output text` step prints one bare value and is addressed by its name alone, so its document is that value.

A labeled `where` is **element-relative**, and that is a correction the re-pin forced rather than a refinement. Upstream writes it as a path over each element the assertion's own projection yields — `credential-report[].mfa_active where password_enabled eq TRUE` means "for each row of the credential report, if it has a console password then MFA is active" — so the assertion's path is split at its last flatten projection, the base yields the elements, each clause filters them one at a time, and the leaf is read from every survivor. `population` stays the count BEFORE the filter, exactly as the row evaluator states it, so "no temporary account among 412 users" reads `0 of 412`. The earlier reading treated a labeled `where` as a GUARD over the whole document, and it failed in both directions: a clause that resolved to nothing made the assertion pass vacuously, and a clause that did resolve let the assertion be judged over the elements it had just excluded — which turned a compliant untagged account into a finding under `temporary-account-automatic-revocation`. A `where` on an assertion whose path has no projection to align on is a defect named, not a pass.
