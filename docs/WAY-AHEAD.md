...
| S4 the surface S1 leans on | S4-1 (#139) done; S4-2 next | S4-1 landed 2026-09-15 (extractor 0.6.0): an opaque `require(expr)` / `import(expr)` is recorded and refuses the negative when reached; `exports` walked whole. #140, #141 open. |
...
1. **S4** first, by the owner's call of 2026-09-15 ("first lets complete our software"): S4-1 (#139) done; **S4-2 (#140)** the lint gate, then S4-3 (#141).
