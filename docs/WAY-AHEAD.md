...
1. **S3** sends, with #72 by 2026-10-09 — S4 closed 2026-09-15 by the owner's call to finish the software first. S3-2 (#138) is researched and drafted in `docs/RESEARCH-S3-2-OUTREACH.md` (channels, a second public package, the sequence, the three drafts); **its §4 corrections come first** (the "only package" claim, the Filevine register section), then the sends wait on the owner for addresses and the go; then S3-3 (#72) and S3-4 (#105).
