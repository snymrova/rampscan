...
| S4 the surface S1 leans on | S4-1 (#139), S4-2 (#140) done; S4-3 next | S4-1 landed 2026-09-15 (extractor 0.6.0): an opaque `require(expr)` / `import(expr)` is recorded and refuses the negative when reached; `exports` walked whole. S4-2 landed 2026-09-15: Biome, `biome.jsonc`, a changed-files job in `check.yml`; 59 console findings inherited and named on touch. #141 open. |
...
1. **S4** first, by the owner's call of 2026-09-15 ("first lets complete our software"): S4-1 (#139) and S4-2 (#140) done; **S4-3 (#141)** the console test floor.
2. **S3** sends, with #72 by 2026-10-09 — S3-0 and S3-1 are done; S3-2 (#138) is researched and drafted in `docs/RESEARCH-S3-2-OUTREACH.md` (channels, a second public package, the sequence, the three drafts) and waits on the owner for addresses and the go; then S3-3 (#72) and S3-4 (#105).
...
pnpm test             # vitest; 1,185 at PR #160
