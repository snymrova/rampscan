...
| S1 reachability soundness | 🔄 3 of 5 | S1-1 done (#145, `c0a4d0a`): absent node → `unknown`, counts, `under_investigation`. S1-2 done (#150, `9e9b347`): SBOM `dependsOn` edges continue the walk, hops marked `sbom`, presence-prover only. S1-3 done (#152, `dfdc80b`; was #151, auto-closed when #150's branch was deleted): application roots recorded in `graph.db`, `not_affected` refused while any root is never entered, `rampscan:scope` on every VEX statement, the SAST gate covered too. The barrel edge in PR #153 (extractor 0.4.0). **Next: S1-4 (#131).** Then #132. S1-5 depends on S1-4: `next` is only imported by `console/web`, outside the configured entry point (finding §4.4 corrected in #150). |
...
1. **Merge the barrel edge (PR #153)** — `export … from "x"` is now an `exact` `imports` edge (extractor 0.4.0), with the four re-export forms tested on a planted barrel package. Measured on this repository: files reached 60 → 109 of 285, every `@rampscan/*` package's `src/` entered in full but one `collectors` file and seven `cli` files; dependency packages reached 12 → 12, so the board does not move here — the shape is closed, the verdicts wait on S1-4. Merge on green; nothing stacks on it.
2. **S1-4 (#131).** `detectEntrypoints` keeps honouring config and reports what package.json detection would have found and config excluded. Measured in #151: this repository has **twelve** application roots and two the walk never enters — `console/web` and the workspace root itself (28 files: PocketBase hooks and migrations, scripts, e2e, the config files). Note that package.json detection finds nothing for `console/web` either (no `main`/`bin`/`exports`; a Next.js app), so S1-4 has to learn `app/layout.tsx` / `next.config.*` or the owner declares the console root in `rampscan.config.json` — either way S1-5 needs it.
3. **S1-5.** S1-4 (and the barrel edge) must land before S1-5 can move `postcss`. S1-5 makes the self-scan *worse* on purpose (12/2 → 11/3); the README must lead with it, not hide it. The invariant S1-2 added (`graph.test.ts`: every package `false` with the SBOM was already `false` without it) is the guard the plan §7.3 asked for — do not weaken it.
