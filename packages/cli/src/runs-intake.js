...
export function classifyStep(step) {
    if (step.stderr_class !== "none")
        return CLASS_OF[step.stderr_class];
 ...
 */
export function rowsOfOutput(bytes, transform) {
 ...
 */
export function outputShape(bytes, transform) {
    let text = new TextDecoder().decode(bytes);
    if (transform === "base64-decode")
    ...
    if (trimmed.startsWith("[") || trimmed.startsWith("{")) {
        try {
            ...
            if (Array.isArray(parsed))
                return { kind: "collection", rows: parsed.filter(isRow) };
            if (isRow(parsed)) {
                const arrays = Object.entries(parsed).filter(([, v]) => Array.isArray(v));
                if (arrays.length === 1)
                    return { kind: "collection", rows: arrays[0][1].filter(isRow) };
 ...
 */
function documentOf(bytes, transform, shape) {
 ...
 */
export function intakeTranscript(transcript, outputs, ctx) {
    ...
    if (transcript.runner.account !== ctx.account || transcript.runner.partition !== ctx.partition) {
        return refuse(`the runner observed account ${transcript.runner.account} (${transcript.runner.partition}); ` +
    ...
    for (const [i, step] of transcript.steps.entries()) {
        ...
        if (cls !== "ok") {
            return {
                kind: "failed",
                class: cls,
                reason: `step ${i + 1} (${step.argv.slice(0, 3).join(" ")}) exited ${step.exit_code} with stderr ${step.stderr_class}: ` +
    ...
    for (const [i, step] of transcript.steps.entries()) {
        ...
        if (shape.kind === "collection")
            collections.push(...shape.rows);
        else if (shape.kind === "record")
            records.push(shape.row);
    ...
    const assertions = ctx.assertions.map((a) => labeledField(a.field, labels) !== undefined
        ? evaluateLabeledAssertions([a], documents, now)[0]
