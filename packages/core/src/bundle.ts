...
export function toEvidenceBundle(
  ...
  return {
    ...
    predicate: {
      ...
      // the evidence-class assertion (Q3.4, G6): everything this pipeline
      // mints is the output of a repeatable process over declared inputs —
      // asserted here, at the bundle's birth, so FRR-PVA-AA-06's standalone
      // rule has a signed fact to read instead of a convention to trust
      evidence_class: "process-generated" as const,
 ...
 * survives and gains pointers only when real drift re-keys it. The Q3.4
 * `evidence_class` assertion is excluded on the same ground: every pipeline
 * bundle is process-generated by construction, so the label adds no
 * information a pipeline bundle didn't already carry implicitly — old
 * evidence honestly carries no assertion until real drift refreshes it.
 *
