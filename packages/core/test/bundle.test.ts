...
import { sameEvidence, toEvidenceBundle } from "../src/bundle.js";
...
function bundle(extra: {
  ...
  evidence_class?: EvidenceBundle["predicate"]["evidence_class"];
}): EvidenceBundle {
  return {
    ...
    predicate: {
      ...
      ...(extra.evidence_class !== undefined ? { evidence_class: extra.evidence_class } : {}),
      cadence: "continuous",
      run_id: "run-1",
...
// The Q3.4 evidence-class assertion rides the I2c slot, not the J5 one: a
// pipeline bundle is process-generated by construction, so the label adds no
// information a pipeline bundle didn't already carry implicitly — a pre-Q3.4
// bundle and its labeled re-scan are the same evidence, and old bundles gain
// the assertion only when real drift re-keys them.
describe("sameEvidence × the Q3.4 evidence-class assertion", () => {
  it("a pre-Q3.4 bundle and its class-asserting re-scan are the same evidence", () => {
    const before = bundle({});
    const after = bundle({ evidence_class: "process-generated" });
    expect(sameEvidence(before, after)).toBe(true);
    expect(sameEvidence(after, before)).toBe(true);
  });

  it("every bundle the pipeline mints asserts process-generated at birth", () => {
    const minted = toEvidenceBundle(
      {
        id: "r",
        ksi_ids: ["KSI-SCR-MIT"],
        control_ids: ["si-7.1"],
      ...
      },
      {
        ...
        control_ids: ["si-7.1"],
        verdict: "evidenced",
        artifacts: [{ name: "facts.json", path: "out/facts.json", sha256: "d".repeat(64) }],
        anchor_paths: [{ path: "f", contentHash: "a".repeat(64) }],
    ...
    expect(minted?.predicate.evidence_class).toBe("process-generated");
  });
});
